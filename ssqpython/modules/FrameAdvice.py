...
class FrameAdvice(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        #命令行提示
        print (u'FrameAdvice启动').encode(locale.getdefaultlocale()[1])
        ...
        #推荐估算
        advice_text = u'%s期推荐号码：'%(int(data_array[0][0])+1) #最终推荐
        ...
        #命令行显示一下
        #print u'本期推荐因子为%d（5为满分）'%(max(in_advice))                
        #print max_num
        #print min_num
        advice_num = [] #推荐号码
        '''
        ...
        advice_text = advice_text + u'本期推荐因子为%d（5为满分）'%(max(in_advice))
